...
void linspace(double array[], double start, double stop, int n) {
    if (n < 0) {
        printf("LINSPACE: n musi być nieujemne!");
        exit(1);
    }
    if (n == 1) {
        array[0] = start;
...
void multiply_by_scalar(double v[], int n, double scalar) {
    if (n < 0) {
        printf("MULTIPLY_BY_SCALAR: n musi być nieujemne!");
        exit(1);
    }
    for (int i = 0; i < n; i++) {
        v[i] *= scalar;
...
void add(double v1[], const double v2[], int n) {
    if (n < 0) {
        printf("ADD: n musi być nieujemne!");
        exit(1);
    }
    for (int i = 0; i < n; i++) {
        v1[i] += v2[i];
...
double dot_product(const double v1[], const double v2[], int n) {
    if (n < 0) {
        printf("DOT_PRODUCT: n musi być nieujemne!");
        exit(1);
    }
    double prod = 0.0;
    for (int i = 0; i < n; i++) {
...
void range(double array[], double start, double step, int n) {
    if (n < 0) {
        printf("RANGE: n musi być nieujemne!");
        exit(1);
    }
    if (n > 0) {
        for (int i = 0; i < n; i++) {
